...
const MensClothingPage = () => {
  ...
  const fetchProducts = async () => {
    try {
      const response = await fetch('/api/products?category=mens-clothing');
      if (response.ok) {
  ...
  const subcategories = [
    ...
    { id: 't-shirts', name: 'T-Shirts', count: products.filter(p => p.subcategory === 't-shirts').length },
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50">
      ...
      <section className="relative py-20 bg-gradient-to-r from-blue-600 to-indigo-600 overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          >
            Elevate your style with our premium collection of men's clothing. From casual essentials to sophisticated formal wear.
          </motion.p>
          <motion.div
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: 1, y: 0 }}
                ...
                <div className={`relative ${viewMode === 'list' ? 'w-48 h-48' : 'h-64'}`}>
                  {product.image_url ? (
                    ...
                    <div className="w-full h-full bg-gradient-to-br from-blue-100 to-indigo-200 flex items-center justify-center">
                      <Shirt className="h-12 w-12 text-blue-400" />
                    </div>
                  )}
        ...
        {filteredProducts.length > 0 && (
          ...
          >
            <motion.button
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-8 py-4 rounded-xl font-semibold hover:from-blue-700 hover:to-indigo-700 transition-all duration-200 shadow-lg"
            >
              Load More Items
            </motion.button>
          </motion.div>
